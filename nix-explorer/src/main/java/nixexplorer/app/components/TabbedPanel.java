...
public class TabbedPanel extends JPanel {
	...
	public TabbedPanel(boolean closable) {
	...
	public int addTab(String title, Icon icon, Component c, boolean switchTo) {
		Tab tab = closable ? new ClosableTab(title, icon, c, this) : new Tab(title, icon, c);
		tabs.add(tab);
		tabPanel.add(tab);
	...
	private boolean canClose(Tab tab) {
		if (listener != null && listener.tabClosing(tabs.indexOf(tab), tab.hashCode())) {
			return true;
		}
		return false;
	}
	...
	static class Tab extends JPanel {
		private JLabel lbl;
		private Color selectionBackground, selectionForeground, background, foreground;
		private Component c;
	...
	static class ClosableTab extends Tab {
		 ...
		 * @param icon
		 */
		public ClosableTab(String title, Icon icon, Component c, TabbedPanel tabbedPanel) {
			super(title, icon, c);
			...
			this.closeButton.setIcon(new BlankIcon(closeIcon.getIconWidth(), closeIcon.getIconHeight()));
